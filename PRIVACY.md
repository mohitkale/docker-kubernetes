# Privacy policy: docker-kubernetes (Claude Code plugin)
...
A developer plugin for **Docker** and **Kubernetes** workflows (for example Dockerfiles, Compose, manifests, Helm, and local debugging guidance).
...
**Session hooks:** Optional Node.js scripts under `hooks/` may read files in your **current working directory** (for example to detect a `Dockerfile`, Compose files, or chart markers) and emit short text as **additional context** inside your Claude Code session. That runs locally where Claude Code is installed. This plugin does not send that output to a separate server operated by the plugin authors.
